...
\begin{itemize}
    ...
    \item \textbf{Como se comunica com outras classes:} depende de \texttt{compression::CompressionRouter} (por referência -- não é dona dele; o \texttt{CompressionRouter} é montado uma única vez, fora daqui, como já vimos no Módulo 3) e possui, por composição, sua própria \texttt{concurrency::ThreadPool}. É consumida pela camada de apresentação (\texttt{cli/}, e futuramente \texttt{gui/}), que é quem efetivamente chama \texttt{adicionar\_job} para cada arquivo encontrado (Módulo 7) e \texttt{cancelar\_job} quando o usuário pede.
...
// (Modulo 3) -- sem duplicar nenhuma regra de roteamento aqui.
...
#include "retroforge/batch/BatchConversionManager.hpp"
#include "retroforge/concurrency/monitors/LinuxProcStatCpuMonitor.hpp"
// ... includes do Modulo 3 (construir_motor_de_recomendacao) omitidos

using namespace retroforge;

int main() {
    auto roteador = construir_motor_de_recomendacao(); // Modulo 3
