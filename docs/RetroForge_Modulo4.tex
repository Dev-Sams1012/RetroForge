...
\begin{itemize}
    ...
    \item \textbf{Como se comunica com outras classes:} depende de \texttt{compression::CompressionRouter} (por referência -- não é dona dele; o \texttt{CompressionRouter} é montado uma única vez, fora daqui, como já vimos no Módulo 3, já com sua trava dupla contra \texttt{DreamcastModificado} e \texttt{PSP\_PBP}) e possui, por composição, sua própria \texttt{concurrency::ThreadPool}. É consumida pela camada de apresentação (\texttt{cli/}, e futuramente \texttt{gui/}), que é quem efetivamente chama \texttt{adicionar\_job} para cada arquivo encontrado (Módulo 7) e \texttt{cancelar\_job} quando o usuário pede.
...
int main() {
    auto roteador = construir_motor_de_recomendacao(); // Modulo 3 -- ja com a
                                                         // trava dupla contra
                                                         // DreamcastModificado
                                                         // e PSP_PBP
